...
public class ServerFacade {
	...
	public Message login(Message m) {
		...
		userID = datas[0]; // keep for later, since we call login with every change of user we looking at
							// anyway
		...
		return result = new Message(MessageType.LOGIN); // i guess we can have client recognized message string = null
														// as failed request.
	}
	...
	public Message getInfo(Message m) {
		...
		if (!authorize(datas[1])) {
			...
			return result = new Message(String.join(",", u.filePrep()), MessageType.UPDATEERROR); // use this to update
																									// user on gui in
																									// case there is a
																									// change to accList
	 ...
	 * // if function not called by super user...this wont be a case since non user
	 * has no access to superuser panel
	 * 
